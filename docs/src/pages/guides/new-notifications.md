...
4. A development environment already set up for javascript/typescript. See the project [README](https://github.com/hyperjumptech/monika/blob/main/README.md) for an overview.
...
Next define all your required data in `data.ts`. If your data is a bit complex, you can define them in a separate module such as `whatsapp.ts`. For our example, google chat only requires a url as input data.
...
The Whatsapp application however, requires a few more data to be passed to it. Below is an example.
...
After defining your interfaces and data types, we'll head over to `src/components/notification/index.ts` to link your notifications API. Find a function called `sendNotifications`, then add your app label you've defined above. This is where we also map the data from Monika, an array called `Notification[]` and an object called `NotificationMessage` which should be passed to your app.
...
Going back to our example for google-chat, we linked `sendNotifications` to our API `sendGoogleChat()` like this:
...
Don't be lazy, add your unit test(s) into the folder `test/components/notification.test.ts` to ensure that all your code behaves as designed. We use `mocha` testing framework and `chai`'s expect for assertion and checking.
